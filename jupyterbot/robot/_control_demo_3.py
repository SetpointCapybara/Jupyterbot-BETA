...
def _control_demo_3():
    ...
    robot = rb.Robot.create_kukakr5(np.identity(4), "robot")
    mesh_obstacle = MeshMaterial(metalness=0.7, clearcoat=1, roughness=0.5, normal_scale=[0.5,0.5], color="green")
    obstacle = Box(htm=Utils.trn([0.09, 0.5, 0.6]), name='obstacle', width=0.14, height=0.14, depth=0.14, mesh_material=mesh_obstacle)
    ...
    sim = Simulation([robot, obstacle, ground, light1, light2, light3, light4, desired_eef_frame], ambient_light_intensity=3)
    ...
    while (not end_loop) and (i < imax):
        ...
        dist_struct = robot.compute_hg_dist(obstacle, h, h, np.array(q), old_dist_struct=dist_struct)
        dist_struct_next = robot.compute_hg_dist(obstacle, h, h, np.array(q + qdot * dt), old_dist_struct=dist_struct)
        ...
        true_dist_struct = robot.compute_hg_dist(obstacle, 0.0001, 0.0001, np.array(q), old_dist_struct=dist_struct)
        ...
        try:
            ...
            hist_dist.append(true_dist_struct[4]["hgDistance"])
            hist_time.append((i - 1) * dt)
            hist_r.append(r)
            hist_q.append(q)
            hist_qdot.append(qdot)
            hist_qddot.append(qddot)
            hp.append(dist_struct[4]["pObj"])
